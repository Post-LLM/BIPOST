...
from bipost.models import DPOLoss, GPTLMLoss
import time
...
class SFT_DPO_ALRIGHT_Trainer(ABC):
    """
    ...
    def fit(self, args):
        ...
        for epoch in range(self.epochs):
            ...
            for step in range(self.sft_train_dataloader.__len__()):
                ...
                # Chose the objective to be updated
                # syncironize the objective index across processes  
                objective_index = np.random.choice([0, 1], p=prob_mass)
               ...
               # DEBUG
                ...
                sft_eval_loss, rlhf_eval_loss = self.save_logs_and_checkpoints(args, global_step, step_bar, logs_dict)
    ...
    def save_logs_and_checkpoints(self, args, global_step, step_bar, logs_dict={}):
        ...
        if global_step % args.eval_steps == 0 or global_step==1:
            sft_eval_loss = self.sft_evaluate(self.sft_eval_dataloader, global_step)
            rlhf_eval_loss = self.rlhf_evaluate(self.rlhf_eval_dataloader, global_step)
        # save ckpt
        ...
        return sft_eval_loss, rlhf_eval_loss
