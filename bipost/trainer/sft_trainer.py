...
from bipost.datasets import SFTDataset
from bipost.models import GPTLMLoss, DPOLoss
...
class SFT_Seq_Trainer(ABC):
    """
        Trainer to use while training SFT model, which will also support evaluation of the model for an RLHF task.
    ...
    def __init__(
        ...
        self.optimizer = optim
        self.args = strategy.args
        self.beta = self.args.rlhf_beta

        self.sft_loss_fn = GPTLMLoss()
    
        self.rlhf_loss_fn = DPOLoss(self.beta, self.args.rlhf_label_smoothing, self.args.rlhf_ipo)
        ...
        # not needed in OPENRLHF version 0.4.5
    ...
    def save_logs_and_checkpoints(self, args, global_step, step_bar, logs_dict={}):
        ...
        if global_step % args.eval_steps == 0 or global_step==1:
            self.sft_evaluate(self.sft_eval_dataloader, global_step)
            self.rlhf_evaluate(self.rlhf_eval_dataloader, global_step)
        # save ckpt
        # TODO: save best model on dev, use loss/perplexity on whole dev dataset as metric
        if global_step % args.save_steps == 0 or global_step==1:
            tag = f"global_step{global_step}"
...
class SFT_Pref_Trainer(ABC):
    """
        Trainer to use while training the model for the chosen response of a preference data set.
    ...
    def __init__(
        ...
        # packing samples not supported (at least for now) for SFT with preference data
        # self.packing_samples = strategy.args.packing_samples
    ...
    def fit(self, args):
        ...
        for epoch in range(self.epochs):
            ...
            self.model.train()
            loss_mean = 0
            for inputs, attention_masks, _, _, prompts_id_lens in self.train_dataloader:
            # for prompts_id_lens, inputs, attention_masks, infos in self.train_dataloader:
                # if self.packing_samples:
                #     inputs = inputs.to(torch.cuda.current_device())
                #     attention_mask = attention_masks.to(torch.cuda.current_device())
                ...
                output = self.model(
                    inputs, attention_mask=attention_mask, return_output=True, #packing_samples=False
                )
    ...
    def evaluate(self, eval_dataloader, steps=0):
        ...
        with torch.no_grad():
            ...
            for inputs, attention_masks, _, _, prompts_id_lens in eval_dataloader:
            # for prompts_id_lens, inputs, attention_masks, infos in eval_dataloader:
                # if self.packing_samples:
                #     inputs = inputs.to(torch.cuda.current_device())
                #     attention_mask = attention_masks.to(torch.cuda.current_device())
                ...
                output = self.model(
                    inputs, attention_mask=attention_mask, return_output=True, packing_samples=False #self.packing_samples
                )
